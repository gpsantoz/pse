...
function createFilter(filter) {
	return function(element, destination) {
		if (element instanceof HTMLVideoElement) {
			if (!element.paused)
				requestAnimationFrame(drawVideo(element, destination));
			return;
		}
				...
				.then(buffer => {
					...
					function buildWam() {
						...
						wam['noise'] = function(pixelData) {
							...
							HEAPF32.set(
								pixelData,
								mem / Float32Array.BYTES_PER_ELEMENT
							);
							_noise(mem, len);
							const filtered = HEAPF32.subarray(
								mem / Float32Array.BYTES_PER_ELEMENT,
								mem / Float32Array.BYTES_PER_ELEMENT + len
							);
							_free(mem);
							return filtered;
						};
						//MultiFilter Family of Functions
						wam['multiFilter'] = function(
							pixelData,
							width,
							filterType,
							mag,
						...
						wam['convFilter'] = function(
							...
							const memData = _malloc(
								arLen * Float32Array.BYTES_PER_ELEMENT
							);
							HEAPF32.set(
								pixelData,
								memData / Float32Array.BYTES_PER_ELEMENT
							);
							const kerWidth = kernel[0].length;
							const kerHeight = kernel.length;
							const kerLen = kerWidth * kerHeight;
							const flatKernel = kernel.reduce((acc, cur) =>
								acc.concat(cur)
							);
							const memKernel = _malloc(
								kerLen * Float32Array.BYTES_PER_ELEMENT
							);
							HEAPF32.set(
								flatKernel,
								memKernel / Float32Array.BYTES_PER_ELEMENT
							);
							_convFilter(
								memData,
								width,
							...
							const filtered = HEAPF32.subarray(
								memData / Float32Array.BYTES_PER_ELEMENT,
								memData / Float32Array.BYTES_PER_ELEMENT + arLen
							);
							_free(memData);
							_free(memKernel);
							return filtered;
						...
						(kernel = [[-1, -1, -1], [-1, 8, -1], [-1, -1, -1]]),
							(divisor = 1);
						wam['strongSharpen'] = bindLastArgs(
							conv,
							kernel,
							divisor
						);
						...
						(kernel = [[-1, -1, 1], [-1, 14, -1], [1, -1, -1]]),
							(divisor = 3);
						wam['goodMorning'] = bindLastArgs(
							conv,
							kernel,
							divisor
						);
						(kernel = [[4, -1, -1], [-1, 4, -1], [0, -1, 4]]),
							(divisor = 3);
						wam['acid'] = bindLastArgs(conv, kernel, divisor);
						(kernel = [[0, 0, -1], [-1, 12, -1], [0, -1, -1]]),
							(divisor = 2);
						wam['dewdrops'] = bindLastArgs(conv, kernel, divisor);
						(kernel = [[-1, -1, 4], [-1, 9, -1], [0, -1, 0]]),
							(divisor = 2);
						wam['destruction'] = bindLastArgs(
							conv,
							kernel,
							divisor
						);
